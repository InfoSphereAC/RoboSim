...
#elif defined(__VEC__)
    uint4(vector int bool vec) : v(vec) {}
#endif
...
#elif defined(__VEC__)
        return vec_any_ne(v, vec_xor(v, v));
#else
		return x || y || z || w;
#endif
	}
...
#elif defined(__VEC__)
        return vec_all_ne(v, vec_xor(v, v));
#else
		return x && y && z && w;
#endif
	}
...
union float4
{
	...
	float operator*(const float4 &other) const { return x*other.x + y*other.y + z*other.z + w*other.w; }
...
#elif defined(__VEC__)
        // Invert result of equal as Altivec has no "not equal". And no bit inversion either.
        return (vector bool int) vec_nor(vec_cmpge(v, other.v), vec_xor(v, v));
#else
		return uint4(x != other.x, y != other.y, z != other.z, w != other.w);
#endif
	}
...
public:
	...
	bool hitsAABB(const float4 &min, const float4 &max, float &entry, float &exit) const;
